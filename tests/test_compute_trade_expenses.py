...
class TestExpenseCalculator(unittest.TestCase):
    def test_delivery_trade_expenses(self):
        result = compute_trade_expenses_detailed(
            qty=100, buy_price=100.0, sell_price=105.0, trade_type="DELIVERY"
        )
        
        # Updated Zerodha DP Charge: ₹13 + 18% GST = ₹15.34
...
class TestPriceForTargetNetPnlPct(unittest.TestCase):
    def test_zero_target_matches_breakeven_price_delivery(self):
        breakeven = compute_trade_expenses_detailed(
            qty=100, buy_price=100.0, sell_price=100.0, trade_type="DELIVERY"
        )["breakeven_price"]
        price = price_for_target_net_pnl_pct(
            qty=100, buy_price=100.0, target_pct=0, trade_type="DELIVERY"
        )
    ...
    def test_stop_loss_2pct_delivery(self):
        price = price_for_target_net_pnl_pct(
            qty=100, buy_price=100.0, target_pct=-2, trade_type="DELIVERY"
        )
        self.assertAlmostEqual(price, 98.37, places=2)
    def test_target_5pct_delivery(self):
        price = price_for_target_net_pnl_pct(
            qty=100, buy_price=100.0, target_pct=5, trade_type="DELIVERY"
        )
